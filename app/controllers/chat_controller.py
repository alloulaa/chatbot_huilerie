...
def ask_chatbot(
    ...
    # Déduire enterprise_id from user_huilerie if not directly available
    if user_huilerie and not user_enterprise_id:
        # Try to extract enterprise_id from the huilerie the user is assigned to
        connection = None
        cursor = None
        try:
            connection = get_db_connection()
            cursor = connection.cursor(dictionary=True)
            query = "SELECT entreprise_id FROM huilerie WHERE LOWER(nom) = LOWER(%s) LIMIT 1"
            cursor.execute(query, (user_huilerie,))
            row = cursor.fetchone()
            if row:
                user_enterprise_id = row.get("entreprise_id")
        except Exception as exc:
            logger.warning("Failed to lookup enterprise_id for huilerie '%s': %s", user_huilerie, exc)
        finally:
            if cursor is not None:
                cursor.close()
            if connection is not None and connection.is_connected():
    ...
    # --- CYCLE DE VIE D'UN LOT -----------------------------------------------
        ...
        if not lot_ref:
            ...
            result = service.get_lot_cycle_vie(lot_reference=lot_ref, enterprise_id=user_enterprise_id)
            if result.get("error"):
