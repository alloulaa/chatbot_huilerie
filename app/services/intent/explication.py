...
def _build_explanation(
    ...
    if nb_issues > 0:
        ...
        if asks_good_yield and normal_yields and not has_low_yield:
            ...
            lines.append(
                f"### 🔍 Conclusion\nLe rendement est globalement bon pour ce lot (moyenne observée : {_fmt(avg_yield, 1)} %), et aucun facteur critique n'a été détecté."
            )
        else:
            lines.append(
...
class ExplicationHandler(IntentHandler):
    ...
    async def handle(self, query: ChatQuery) -> IntentResult:
        ...
        asks_good_yield = (
            ...
            and any(token in message_lower for token in ["bon", "bonne", "correct", "eleve", "élevé"])
        )
